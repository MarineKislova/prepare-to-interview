...
const questionsJSHabrPart1 = [
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "habr.com",
      answer1:
        "<p>Основное отличие между Set и WeakSet заключается в типе значений, которые они могут содержать, и в поведении при сборке мусора. Set может содержать любые значения и не удаляет их автоматически, а WeakSet может содержать только объекты и может быть автоматически очищен сборщиком мусора, если на объект больше нет ссылок.</p> <p>Еще можно почитать <a href='https://learn.javascript.ru/weakmap-weakset' target='_blank' rel='noopener noreferrer'>здесь</a> и <a href='https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/WeakMap' target='_blank' rel='noopener noreferrer'>здесь</a>.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "habr.com",
      answer1:
        "<p>Сборщик мусора в JavaScript является встроенной функцией, которая автоматически освобождает память, занятую объектами, которые больше не используются в программе. Он следит за объектами, которые были созданы во время выполнения программы, и определяет, когда они больше не доступны для использования.</p> <p>Сборщик мусора в JS использует алгоритм под названием 'Mark and Sweep' (Пометка и Очистка). Вот как это работает:</p> <ul> <li>Пометка (Mark): Сборщик мусора начинает с корневых объектов, таких как глобальный объект (window в браузере) и все объекты, на которые есть ссылки из корневых объектов. Он помечает эти объекты как активные.</li> <li>Распространение (Propagation): Сборщик мусора рекурсивно проходит через все активные объекты и помечает объекты, на которые они ссылаются, как активные. Этот процесс продолжается до тех пор, пока все достижимые объекты не будут помечены.</li> <li>Очистка (Sweep): После завершения пометки и распространения, сборщик мусора проходит по всей памяти и освобождает память, занятую не помеченными объектами. Он удаляет эти объекты и восстанавливает память для будущего использования.</li> </ul> <p>Еще можно почитать <a href='https://learn.javascript.ru/garbage-collection' target='_blank' rel='noopener noreferrer'>здесь</a> и <a href='https://developer.mozilla.org/ru/docs/Web/JavaScript/Memory_management' target='_blank' rel='noopener noreferrer'>здесь</a>.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
...
const questionsJSHabr35 = [
  ...
  },
  {
    ...
    answer: {
      href: "https://habr.com/ru/articles/578370/",
      linkName: "35 вопросов с собеседования JavaScript-разработчика",
      answer1: "<p>Для этого можно использовать встроенный метод Array.isArray().</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
