...
const sortedQuestionsJS = [
  ...
  },
  {
    id: 4,
    question:
      "Что произойдет в результате выражения var foo = 10 + '20' и почему? (приведение типов и преобразование типов)",
    answer: {
      href: "",
      linkName: "",
      answer1:
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p><span>null</span> - это в прямом смысле 'ничего'. Можно использовать это значение в качестве плейсхолдера.</p> <p><span>undefined</span> - это значение для случая 'объявлено, но не инициализировано'.</p> <p><span>undeclared</span> - это ошибка, возникающая в случае обращении к необъявленной переменной.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p><span>undefined</span> - это значение, присваемое объявленной, но не проинициализированной переменной. Мы получаем <span>undefined</span>, обращаясь к существующей переменной. А в случае обращения к несуществующей (необъявленной) переменной, мы получим ошибку <span>is not defined</span></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Следует использовать <span>функцию Boolean:</span> <code>const res1 = new Boolean('');  //[Boolean: false] <br> const res2 = new Boolean(0); //[Boolean: false] <br> const res3 = new Boolean(null); //[Boolean: false] <br> const res4 = new Boolean(undefined); //[Boolean: false] <br> const res5 = new Boolean(NaN); //[Boolean: false] <br> const res6 = new Boolean(false); //[Boolean: false]</code>  или <span>оператор '!!'</span> (двойное отрицание): <code>console.log(!!''); // => false<br>console.log(!!0); // => false<br>console.log(!!null); // => false <br>console.log(!!undefined); // => false<br>console.log(!!NaN); // => false<br>console.log(!!false); // => false</code></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
...

  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://doka.guide/js/var-let/' target='_blank' rel='noopener noreferrer'>Переменные const, let и var</a></p><p>Переменные, объявленные с помощью ключевого слова <span>«var»</span>, являются глобальными. Это означает, что они доступны из любого места в коде. Переменные <span>var</span> можно объявлять без присвоения им значения, в таком случае они будут равны <span>undefined</span>. К переменным, объявленным при помощи ключевого слова <span>var</span>, можно обращаться до момента объявления. В отличие от <span>let</span> и <span>const</span>, ошибки это не вызовет. Такое поведение называется <span>hoisting - «всплытие»:</span></p> <code>console.log(a) // undefined <br> var a = 5 <br> console.log(a) // 5</code> <p>Переменные, объявленные через <span>var</span>, имеют функциональную область видимости. Они доступны только в пределах текущей функции или глобального объекта, если функции нет: <code>if (true) { <br>   var a = 5 <br>} <br> function foo() { <br> var b = 10 <br>} <br>  console.log(a) // 5 <br> console.log(b) // ReferenceError: b is not defined </code></p> <p>Переменные, объявленные с помощью ключевых слов <span>«let»</span> и <span>«const»</span> имеют блочную область видимости и не становятся частью глобального объекта (window в браузере, global в Node.js). Блочная область видимости не даёт получить значение переменной вне блока, где она была объявлена. Это означает, что они доступны только внутри блока <span>({ })</span>.</p> <p>Разница между <span>«let»</span> и <span>«const»</span> состоит в том, что в первом случае мы может менять значение переменной, а во втором — нет (константа). При этом, мы можем менять значение свойства объекта, объявленного с помощью <span>const</span>, но не само свойство (переменную).</p><code>if (true) { <br> var a = 5 <br> let b = 10 <br> const c = 15 <br>  console.log(a) // 5 <br>   console.log(b) // 10 <br>  console.log(c) // 15 <br> } <br> console.log(a) // 5 <br> console.log(b) // ReferenceError: a is not defined <br> console.log(b) // ReferenceError: b is not defined</code> ",
      image1: "",
      href2: "",
      linkName2: "",
      answer2:
        "<p><span style='font-style: italic;'>var:</span></p> <ul> <li>Переменные, объявленные с помощью <span>var</span>, имеют функциональную область видимости или область видимости внутри целого файла.</li> <li>Переменные var могут быть переопределены и переобъявлены в той же области видимости.</li> <li>Переменные <span>var</span> поднимаются <span>(hoisted)</span> в начало своей области видимости, что означает, что их можно использовать до их фактического объявления.</li> </ul> <p><span style='font-style: italic;'>let:</span></p> <ul> <li>Переменные, объявленные с помощью <span>let</span>, имеют блочную область видимости, ограниченную фигурными скобками <span>{}</span>.</li> <li>Переменные <span>let</span> не могут быть переобъявлены в той же области видимости, но могут быть переопределены.</li> <li>Переменные <span>let</span> не поднимаются (не hoisted) и не могут быть использованы до их объявления.</li> </ul> <p><span style='font-style: italic;'>const:</span></p> <ul> <li>Переменные, объявленные с помощью <span>const</span>, также имеют блочную область видимости.</li> <li>Переменные <span>const</span> должны быть инициализированы при объявлении и их значение не может быть изменено после этого.</li> <li>Переменные <span>const</span> не могут быть переобъявлены или переопределены.</li> <li><span>const</span> также создает неизменяемую ссылку на объект, поэтому значения внутри объекта могут быть изменены, но сама ссылка остается неизменной.</li> </ul>",
      image2: "",
      href3: "",
      linkName3: "",
      answer3: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://doka.guide/js/closures/' target='_blank' rel='noopener noreferrer'>Области видимости</a></p><p><span>Область видимости</span> — это место, где (или откуда) мы имеем доступ к переменным или функциям. JS имеем три типа областей видимости: глобальная, функциональная и блочная (ES6).</p> <p><span>Глобальная область видимости</span> — переменные и функции, объявленные в глобальном пространстве имен, имеют глобальную область видимости и доступны из любого места в коде.</p> <p><img class='question__img' src='img/habr70/19.1habr70.png' alt=''></p> <p><span>Функциональная область видимости</span> (область видимости функции) — переменные, функции и параметры, объявленные внутри функции, доступны только внутри этой функции.</p> <p><img class='question__img' src='img/habr70/19.2habr70.png' alt=''></p> <p><span>Блочная область видимости</span> — переменные (объявленные с помощью ключевых слов «let» и «const») внутри блока ({ }), доступны только внутри него.</p> <p><img class='question__img' src='img/habr70/19.3habr70.png' alt=''></p> <p><span>Область видимости</span> — это также набор правил, по которым осуществляется поиск переменной. Если переменной не существует в текущей области видимости, ее поиск производится выше, во внешней по отношению к текущей области видимости. Если и во внешней области видимости переменная отсутствует, ее поиск продолжается вплоть до глобальной области видимости. Если в глобальной области видимости переменная обнаружена, поиск прекращается, если нет — выбрасывается исключение. Поиск осуществляется по ближайшим к текущей областям видимости и останавливается с нахождением переменной. Это называется цепочкой областей видимости (Scope Chain).</p> <p><img class='question__img' src='img/habr70/19.4habr70.png' alt=''></p> <p><img class='question__img' src='https://habrastorage.org/r/w1560/webt/r6/07/q7/r607q7hehfjmuicf0_icntddk2s.png' alt=''></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://developer.mozilla.org/ru/docs/Web/JavaScript/Guide/Closures' target='_blank' rel='noopener noreferrer'>Замыкания</a></p><p><span>Замыкание</span> — это комбинация функции и лексического окружения, в котором эта функция была определена. Другими словами, замыкание даёт вам доступ к Scope внешней функции из внутренней функции. В JavaScript замыкания создаются каждый раз при создании функции, во время её создания.</p> <p><span>Лексическая область видимости</span></p><p><img class='question__img' src='img/habr70/20.1habr70.png' alt=''></p> <p><span>init()</span> создаёт локальную переменную name и определяет функцию <span>displayName()</span>. <span>displayName()</span> — это внутренняя функция — она определена внутри <span>init()</span> и доступна только внутри тела функции <span>init()</span>. Обратите внимание, что функция <span>displayName()</span> не имеет никаких собственных локальных переменных. Однако, поскольку внутренние функции имеют доступ к переменным внешних функций, <span>displayName()</span> может иметь доступ к переменной name, объявленной в родительской функции <span>init()</span>.</p> <p>Выполните этот код и обратите внимание, что команда alert() внутри displayName() благополучно выводит на экран содержимое переменной name объявленной в родительской функции. Это пример так называемой лексической области видимости (lexical scoping): в JavaScript область действия переменной определяется по её расположению в коде (это очевидно лексически), и вложенные функции имеют доступ к переменным, объявленным вовне. Этот механизм и называется Lexical scoping (область действия, ограниченная лексически).</p> <p><span>Замыкание</span></p> <p>Рассмотрим следующий пример:</p> <p><img class='question__img' src='img/habr70/20.2habr70.png' alt=''></p> <p>На первый взгляд, кажется неочевидным, что этот код правильный, но он работает. В некоторых языках программирования локальные переменные-функции существуют только во время выполнения этой функции. После завершения выполнения <span>makeFunc()</span> можно ожидать, что переменная name больше не будет доступна. Однако, поскольку код продолжает нормально работать, очевидно, что это не так в случае JavaScript.</p> <p>Причина в том, что функции в <span>JavaScript</span> формируют так называемые <span>замыкания</span>. <span>Замыкание</span> — это комбинация функции и лексического окружения, в котором эта функция была объявлена. Это окружение состоит из произвольного количества локальных переменных, которые были в области действия функции во время создания замыкания. В рассмотренном примере <span>myFunc</span> — это ссылка на экземпляр функции <span>displayName</span>, созданной в результате выполнения <span>makeFunc</span>. Экземпляр функции <span>displayName</span> в свою очередь сохраняет ссылку на своё лексическое окружение, в котором есть переменная <span>name</span>. По этой причине, когда происходит вызов функции <span>myFunc</span>, переменная <span>name</span> остаётся доступной для использования и сохранённый в ней текст 'Mozilla' передаётся в alert.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Дополнительно о Hoisting можно почитать <a class='question__link' href='https://developer.mozilla.org/ru/docs/Glossary/Hoisting' target='_blank' rel='noopener noreferrer'>здесь</a></p><p><span>Поднятие</span> — это термин, описывающий подъем переменной или функции в глобальную или функциональную области видимости.</p> <p>Для того, чтобы понять, что такое <span>Hoisting</span>, необходимо разобраться с тем, что представляет собой контекст выполнения.</p> <p><span>Контекст выполнения</span> — это среда, в которой выполняется код. Контекст выполнения имеет две фазы — компиляция и собственно выполнение.</p> <p><span>Компиляция. </span>В этой фазе функции, объявленные через <span>Function Declaration</span>, поднимаются вместе с их телом; и переменные, объявленные с помощью ключевого слова «var», со значением undefined поднимаются в самый верх глобальной (или функциональной) области видимости (как бы перемещаются в начало нашего кода. Это объясняет, почему мы можем вызывать функции до их объявления — прим. пер.).</p> <p><span>Выполнение.</span> В этой фазе переменным присваиваются значения, а функции (или методы объектов) вызываются или выполняются.</p> <p><span>Запомните:</span> поднимаются только обычные функции (Function Declaration) и переменные, объявленные с помощью ключевого слова «var». Функциональные выражения (Function Expression) и стрелочные функции (Arrow Function), а также переменные, объявленные с помощью ключевых слов «let» и «const» не поднимаются.</p> <p>После завершения фазы компиляции начинается фаза выполнения, когда переменным присваиваются значения и вызываются функции.</p> ",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "Оператор '&&' (логическое и) находит и возвращает первое ложное значение либо последний операнд, когда все значения истинные. Он использует короткое замыкание во избежание лишних затрат: <code>console.log(false && 1 && []) // false, <br> console.log(' ' && true && 5) // 5</code> При этом второй операнд не вычисляется.",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p><span>Оператор '!!'</span> <span style='font-style: italic;'>(двойное отрицание)</span> приводит значение справа от него к логическому значению.</p><code>console.log(!!null) // false,<br>console.log(!!undefined) // false,<br>console.log(!![])  //true,<br>console.log(!!1)  //true</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Разница между <span>оператором '=='</span> (абстрактное или нестрогое равенство) и <span>оператором '==='</span> (строгое равенство) состоит в том, что: <br> - <span>оператор '=='</span> сравнивает значения <span style='font-style: italic;'>после их преобразования или приведения к одному типу (Coersion)</span>, например, если вы сравниваете строку с числом, JavaScript преобразует строку в число перед сравнением, <br> - <span>оператор '==='</span> </span> — <span style='font-style: italic;'>без такого преобразования, </span>то есть если строка не может быть преобразована в число, она преобразуется в NaN, что возвращает false.</p>  <p><span>Запомните:</span> <span style='font-style: italic;'>для приведения объекта к «примитиву» <span>метод toPrimitive</span> сначала использует <span>метод valueOf</span>, затем <span>метод toString</span>.</span></p> <p>Если оба операнда имеют <span>разные</span> типы данных, но они могут быть преобразованы в один и тот же <span>тип</span> данных и имеют одно и то же <span>значение</span>, оператор == вернет <span>true</span>:</p> <code>  var a = 10; <br>  console.log(a == 20); // false <br>  console.log(a == '1o'); // false <br>  console.log(a == '10'); // true</code> <p>Если два значения имеют <span style='font-style: italic;'>разные типы данных</span>, <span>оператор === вернет false</span>, даже если они имеют одно и то же значение. Если оба операнда имеют <span style='font-style: italic;'>одинаковый тип данных</span> и <span style='font-style: italic;'>одинаковое  значение</span>, <span>оператор === вернет true</span>:</p><code>  var x = 10; <br>  console.log(x === '10'); // false <br>  console.log(x === 10); // true</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Результатом сравнения двух похожих объектов в JavaScript может быть <span>false</span>, потому что сравнение объектов происходит по ссылке, а не по содержимому.</p> <p>Когда мы сравниваем два объекта, JavaScript проверяет, указывают ли оба операнда на один и тот же объект в памяти. Если это так, то результатом будет <span>true</span>. Однако, если операнды указывают на разные объекты, даже если они имеют одинаковую структуру и значения свойств, результатом будет <span>false</span>.</p> <code>const obj1 = { name: 'John', age: 25 }; <br> const obj2 = { name: 'John', age: 25 }; <br> const obj3 = obj2; <br> console.log(obj1 === obj2); //false <br> console.log(obj3 === obj2); //true</code> <p>В этом примере obj1 и obj2 содержат <span style='font-style: italic;'>одинаковые свойства и значения</span>, но они являются <span style='font-style: italic;'>разными объектами в памяти</span>. Поэтому результатом сравнения <span>obj1 === obj2 будет false</span>.</p> <p>Если вам нужно сравнить содержимое двух объектов, вам придется реализовать собственную логику сравнения, перебирая свойства объектов и сравнивая их значения.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Согласно MDN оператор ' + ' действительно является самым быстрым способом преобразования строки в число, поскольку он не выполняет никаких операций со значением, которое является числом.</p> ",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://learn.javascript.ru/dom-nodes' target='_blank' rel='noopener noreferrer'>DOM-дерево</a>, <a class='question__link' href='https://proglib.io/p/dom-chto-takoe-obektnaya-model-dokumenta-prostymi-slovami-2022-02-18' target='_blank' rel='noopener noreferrer'>DOM: что такое объектная модель документа простыми словами</a></p> <p><span>DOM или Document Object Model</span> <span style='font-style: italic;'>(объектная модель документа)</span> — это прикладной программный интерфейс (API) для работы с HTML и XML документами. Когда браузер первый раз читает («парсит») HTML документ, он формирует большой объект, действительно большой объект, основанный на документе — <span></span>DOM. <span>DOM</span> представляет собой древовидную структуру (дерево документа). <span>DOM</span> используется для взаимодействия и изменения самой <span>структуры DOM</span> или его отдельных элементов и узлов.</p> <p>Основой <span>HTML-документа</span> являются теги. В соответствии с <span style='font-style: italic;'>объектной моделью документа</span> (<span>«Document Object Model», коротко DOM</span>), каждый <span>HTML-тег</span> является объектом. Вложенные теги являются «детьми» родительского элемента. Текст, который находится внутри тега, также является объектом. Все эти объекты доступны при помощи <span>JavaScript</span>, мы можем использовать их для изменения страницы, а это позволяет динамически управлять содержимым страницы.</p> <p></p> <p></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://learn.javascript.ru/shadow-dom' target='_blank' rel='noopener noreferrer'>Shadow DOM</a></p><p><span>Теневой DOM («Shadow DOM»)</span> используется для инкапсуляции. Благодаря ему в компоненте есть собственное «теневое» DOM-дерево, к которому нельзя просто так обратиться из главного документа, у него могут быть изолированные CSS-правила и т.д.</p> <p><span>Встроенный теневой DOM</span> Задумывались ли вы о том, как устроены и стилизованы сложные браузерные элементы управления? Например, input type='range': <input type='range'></p> <p>Браузер рисует их своими силами и по своему усмотрению. Их DOM-структура обычно нам не видна, но в инструментах разработчика можно её посмотреть. К примеру, в Chrome для этого нужно активировать <span>пункт «Show user agent shadow DOM»</span>.</p> <p>Мы не можем получить доступ к теневому DOM встроенных элементов с помощью обычных JavaScript-вызовов или с помощью селекторов. Это не просто обычные потомки, это мощное средство инкапсуляции (изоляции).</p><p><span>Shadow DOM</span> – это техника, позволяющая создавать изолированные фрагменты HTML и CSS в специальном сегменте DOM, который находится внутри определенного элемента. Такой подход позволяет исключить влияние инкапсулированных стилей на структуру и внешний вид основной страницы, и обеспечивает большую гибкость и контроль над представлением и поведением элементов.</p> <p>В приведенном ниже примере инкапсулированные стили пользовательского элемента не влияют на внешний вид остальной части страницы</p>  <p><img class='question__img' src='img/25proglibPart2.png' alt=''></p> <img class='question__img' src='https://media.proglib.io/posts/2024/01/10/9a73b0cff11a40d5727227b04360a489.png' alt=''>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее: <a class='question__link' href='https://learn.javascript.ru/bubbling-and-capturing' target='_blank' rel='noopener noreferrer'>Всплытие и погружение</a> и <a class='question__link' href='https://www.w3.org/TR/uievents/#event-flow' target='_blank' rel='noopener noreferrer'>здесь</a></p><p>Когда какое-либо событие происходит в элементе DOM, оно на самом деле происходит не только в нем. Событие «распространяется» от объекта Window до вызвавшего его элемента (event.target). При этом событие последовательно пронизывает (затрагивает) всех предков целевого элемента. Распространение события имеет три стадии или фазы:</p> <ol> <li><span>Фаза погружения</span> (захвата, перехвата) — событие возникает в объекте Window и опускается до цели события через всех ее предков.</li> <li><span>Целевая фаза</span> — это когда событие достигает целевого элемента.</li> <li><span>Фаза всплытия</span> — событие поднимается от event.target, последовательно проходит через всех его предков и достигает объекта Window.</li> </ol> <p><img class='question__img' src='https://habrastorage.org/r/w1560/webt/a3/9x/67/a39x67wkxdmcyzj-it5m2jmfxgg.png' alt=''></p> <p><span>Обратите внимание, что хоть и формально существует 3 фазы, 2-ую фазу («фазу цели»: событие достигло элемента) нельзя обработать отдельно, при её достижении вызываются все обработчики: и на всплытие, и на погружение.</span></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://learn.javascript.ru/bubbling-and-capturing#vsplytie' target='_blank' rel='noopener noreferrer'>Всплытие</a></p> <p>Когда событие происходит в элементе DOM, оно затрагивает не только этот элемент. Событие «всплывает» (подобно пузырьку воздуха в воде), переходит от элемента, вызвавшего событие <span>(event.target)</span>, к его родителю, затем поднимается еще выше, к родителю родителя элемента, пока не достигает <span>объекта Window</span>.</p> <p>Допустим, у нас есть такая разметка:  <p><img class='question__img' src='img/habr70/2.1habr70.png' alt=''></p> <p> Поэтому если кликнуть на child, то мы увидим три оповещения: <span>child → parent → grandparent</span>. Этот процесс называется <span>«всплытием»</span>, потому что события «всплывают» от внутреннего элемента вверх через родителей подобно тому, как всплывает пузырёк воздуха в воде.</p> <p>Почти все события всплывают. Ключевое слово в этой фразе – «почти». Например, событие focus не всплывает. В дальнейшем мы увидим и другие примеры. Однако, стоит понимать, что это скорее исключение, чем правило, всё-таки большинство событий всплывают.</p> <p>Всегда можно узнать, на каком конкретно элементе произошло событие. Самый глубокий элемент, который вызывает событие, называется целевым элементом, и он доступен через <span>event.target</span>. Отличия от <span>this (=event.currentTarget)</span>:</p> <p>1.<span> event.target</span> – это «целевой» элемент, на котором произошло событие, в процессе всплытия он неизменен.</p> <p>2. <span>this</span> – это «текущий» элемент, до которого дошло всплытие, на нём сейчас выполняется обработчик. Например, если стоит только один обработчик <span>grandparent.onclick</span>, то он «поймает» все клики внутри формы. Где бы ни был клик внутри – он всплывёт до <span>элемента grandparent</span>, на котором сработает обработчик.</p> <p><span>Обратите внимание, что хоть и формально существует 3 фазы, 2-ую фазу («фазу цели»: событие достигло элемента) нельзя обработать отдельно, при её достижении вызываются все обработчики: и на всплытие, и на погружение.</span></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://learn.javascript.ru/bubbling-and-capturing#pogruzhenie' target='_blank' rel='noopener noreferrer'>Погружение</a></p><p>Когда событие происходит в элементе DOM, оно происходит не только в нем. В фазе погружения событие опускается от объекта Window до цели события через всех его предков</p> <p>Допустим, у нас есть такая разметка:</p> <p><img class='question__img' src='img/habr70/2.1habr70.png' alt=''></p>  <p>То есть при клике на 'parent' событие путешествует по цепочке родителей сначала вниз к элементу (погружается), затем оно достигает целевой элемент (фаза цели), а потом идёт наверх (всплытие), вызывая по пути обработчики.</p>  <p>У метода addEventListener есть третий необязательный параметр — useCapture. Когда его значение равняется false (по умолчанию), событие начинается с фазы всплытия. Когда его значение равняется true, событие начинается с фазы погружения (для «прослушивателей» событий, прикрепленных к цели события, событие находится в целевой фазе, а не в фазах погружения или всплытия. События в целевой фазе инициируют все прослушиватели на элементе в том порядке, в котором они были зарегистрированы независимо от параметра useCapture — прим. пер.). Если мы кликнем по элементу child, в консоль будет выведено: child, parent, grandparent, html, document, window. Вот что такое всплытие события.</p> <code>for(let elem of document.querySelectorAll('*')) {<br>   elem.addEventListener('click', e => alert(`Погружение: ${elem.tagName}`), true); <br>  elem.addEventListener('click', e => alert(`Всплытие: ${elem.tagName}`));<br>}</code> <p><span>Обратите внимание, что хоть и формально существует 3 фазы, 2-ую фазу («фазу цели»: событие достигло элемента) нельзя обработать отдельно, при её достижении вызываются все обработчики: и на всплытие, и на погружение.</span></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://learn.javascript.ru/bubbling-and-capturing#prekraschenie-vsplytiya' target='_blank' rel='noopener noreferrer'>Прекращение всплытия,</a> <a class='question__link' href='https://learn.javascript.ru/default-browser-action#otmena-deystviya-brauzera' target='_blank' rel='noopener noreferrer'>Отмена действия браузера</a></p> <p>Методы <span>event.preventDefault()</span> и <span>event.stopPropagation()</span> предназначены для решения похожих задач и часто используются вместе.</p> <p><span>Метод event.stopPropagation()</span> останавливает распространение события <span>(event propagation)</span>. Всплытие идёт с «целевого» элемента прямо наверх. По умолчанию событие будет всплывать до элемента <html>, а затем до объекта document, а иногда даже до window, вызывая все обработчики на своём пути. Но любой промежуточный обработчик может решить, что событие полностью обработано, и остановить всплытие. Для этого нужно вызвать <span>метод event.stopPropagation()</span>.</p> <p>Например, рассмотрим обработку события на стадии захвата — при распространении события от родительских элементов к дочерним. Обработчик, объявленный в дочернем элементе, не будет вызван после выполнения <span>stopPropagation()</span> в обработчике родительского элемента. При этом действие по умолчанию не отменится. <p>Вызов <span>event.preventDefault()</span> является возможностью для обработчика события сообщить в сгенерировавший событие код, что эти действия надо отменить. <span>Метод event.preventDefault()</span> отменяет действие по умолчанию.</p> </p> <p> Например, переход по ссылке при клике на элемент <span><</span><span>a></span>. Если использовать этот метод в элементе form, то он предотвратит отправку формы (submit). Само событие продолжает распространяться после выполнения <span>preventDefault()</span>.</p> ",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Для этого мы можем использовать свойство <span>event.defaultPrevented</span>, возвращающее логическое значение, служащее индикатором применения к элементу метода <span>event.preventDefault</span>.</p> <p>Например, есть такая разметка:</p> <p><img class='question__img' src='img/defaultPrevented.png' alt=''></p> <p>Свойство события <span>defaultPrevented</span> проверяет, был ли вызван <span>метод preventDefault()</span> для события.</p> <code>document.getElementById('myAnchor').addEventListener('click', function (event) {<br>   event.preventDefault();<br>   console.log('Was preventDefault() called: ' + event.defaultPrevented); <span style='font-style: italic;'>//Was preventDefault() called: true</span> });<br> <br> document.querySelector('.a-link').ddEventListener('click/, function (event) {<br>   event.preventDefault();<br>   if (event.defaultPrevented) {<br><span style='font-style: italic;'>/* по умолчанию не выполнится */</span>  console.log(event.defaultPrevented); <span style='font-style: italic;'>//true</span>   } <br>});</code> <p><span>Возвращаемое значение:</span>	Логическое значение, указывающее, был ли вызван <spanметод preventDefault()span> для события.</p> <p><span>- true</span> — метод preventDefault() был вызван для события</p> <p><span>- false</span> — метод preventDefault() не был вызван для события.</p> <p></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Простыми словами, <span>event.target</span> — это элемент, в котором происходит событие, или элемент, вызвавший событие.</p> <p>Например, у нас есть такая разметка:</p> <p><img class='question__img' src='img/eventTarget.png' alt=''></p> <p>JavaScript:</p> <code>let parent = document.querySelector('.form__container');<br> let child = document.querySelector('.form__text');<br> <br> parent.addEventListener('click', function (event) {<br>   child = event.target;<br>   event.target.style.backgroundColor = 'red';<br>   event.currentTarget.style.backgroundColor = 'yellow';<br> });</code> <p>При нажатии на child <span>event.target</span> подсветится красным цветом, то есть целевой элемент form__text.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p><span>Event.currentTarget</span> — это элемент, к которому прикреплен прослушиватель событий. <span>Event.currentTarget</span> будет указывать на родительский элемент.</p> <p>Например, у нас есть такая разметка:</p> <p><img class='question__img' src='img/eventTarget.png' alt=''></p> <p>JavaScript:</p> <code>let parent = document.querySelector('.form__container');<br> let child = document.querySelector('.form__text');<br> <br> parent.addEventListener('click', function (event) {<br>   child = event.target;<br>   event.target.style.backgroundColor = 'red';<br>   event.currentTarget.style.backgroundColor = 'yellow';<br> });</code> <p>При нажатии на child <span>event.currentTarget</span> подсветится желтым цветом, то есть родительский элемент form__container.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p><span>Шаблонные литералы (template literals)</span> — относительно новый способ создания строк в JS. Шаблонные литералы создаются с помощью двойных обратных кавычек (``)</p> <p><img class='question__img' src='img/habr70/45habr70.png' alt=''></p> <p>Шаблонные строки также поддерживают интерполяцию строк, что позволяет вставлять выражения прямо в строку. Эти выражения заключаются в фигурные скобки ${expression} и вычисляются при создании строки</p> <p><img class='question__img' src='img/7.2proglibPart1.png' alt=''></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Оператор опциональной последовательности ?. позволяет получить безопасный доступ к вложенным свойствам объекта – даже в том случае, когда промежуточное свойство отсутствует. Оператор ?.прекращает оценку и возвращает undefined, если часть после ?. является либо undefined, либо null.</p> <p><img class='question__img' src='img/24proglibPart2.png' alt=''></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://proglib.io/p/shpargalka-po-regulyarnym-vyrazheniyam-v-javascript-2022-07-17' target='_blank' rel='noopener noreferrer'>Шпаргалка по регулярным выражениям в JavaScript</a></p> <p><span>Регулярные выражения (regex)</span> – это паттерны, которые используются для поиска и замены текста в строках. В паттернах используются комбинации специальных символов (шаблоны) и флаги (модификаторы, которые определяют поведение поиска). Регулярные выражения часто используют в комбинации с этими методами:</p> <ul> <li><span>test()</span> – проверка на соответствие шаблону</li> <li><span>match()</span> – поиск соответствий</li> <li><span>replace()</span> – замена по шаблону</li> <li><span>search()</span> – поиск индекса</li> </ul> <p>Составлять регулярные выражения можно с помощью <span>конструктора RegExp</span> или <span>литеральной нотации</span>.</p> ",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Команда <span>use strict</span> включает так называемый строгий режим. В этом режиме предупреждения становятся ошибками, что помогает в написании более чистого и безопасного кода, менее уязвимого к ошибкам в продакшене.</p> <p>Команда <span>«use strict»</span> — это директива ES5, которая заставляет весь наш код или код отдельной функции выполняться в строгом режиме. Строгий режим вводит некоторые ограничения по написанию кода, тем самым позволяя избегать ошибок на ранних этапах.</p> <p>Вот какие ограничения накладывает строгий режим:</p> <p>1. Нельзя присваивать значения или обращаться к необъявленным переменным </p> <p>2. Запрещено присваивать значения глобальный переменным, доступным только для чтения или записи</p> <p>3. Нельзя удалить «неудаляемое» свойство объекта</p>  <p>4. Запрещено дублирование параметров</p>  <p>5. Нельзя создавать функции с помощью функции eval</p> <p>6. Значением «this» по умолчанию является undefined</p> ",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  {
    id: 40,
    question: "Какое значение имеет this? Что такое this?",
    answer: {
      href: "",
      linkName: "",
      answer1:
        "<p><span>this</span> - это специальное ключевое слово в JavaScript, которое ссылается на объект, в контексте которого выполняется текущий код. Значение this зависит от контекста вызова функции и может быть разным в различных ситуациях.</p> <p>Обычно <span>this</span> ссылается на значение объекта, который в данный момент выполняет или вызывает функцию. «В данный момент» означает, что значение <span>this</span> меняется в зависимости от контекста выполнения, от того места, где мы используем <span>this</span></p> <p><img class='question__img' src='img/habr70/24habr70.png' alt=''></p> <p>В данном случае метод getName возвращает this.name, а this ссылается на carDetails, объект, в котором выполняется getName, который является ее «владельцем».</p> <p><span>this</span> указывает на объект области видимости во время выполнения. По <span>умолчанию this</span> указывает на глобальный объект. В браузере выражение <span>this === window</span> будет истинно.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  {
    id: 41,
    question: "Отличие this в function declaration и стрелочных функциях",
    answer: {
      href: "",
      linkName: "",
      answer1:
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Подробнее тут: <a class='question__link' href='https://developer.mozilla.org/ru/docs/Learn_web_development/Extensions/Advanced_JavaScript_objects/Object_prototypes' target='_blank' rel='noopener noreferrer'>Прототипы объектов</a></p> <p><span>Прототипы</span> - это механизм, с помощью которого объекты JavaScript наследуют свойства друг от друга.</p> <p>Пример:</p> <code>function Person(first, last, age, gender) { <br>  // Определения методов и свойств <br>   this.name = {<br>     first: first,<br>     last: last, <br>   };<br>   this.age = age; <br>   this.gender = gender; <br> }</code> <p>Затем мы создаём экземпляр объекта следующим образом:</p> <code>var person1 = new Person('Bob', 'Smith', 32, 'male'); <br> console.log(person1);</code> <p>Если вывести в консоль person1 можно увидеть, что person1 заполнено с именами участников, доступных на этом объекте</p> <p>JavaScript часто описывают как <span>язык прототипного наследования</span> — каждый объект, имеет объект-прототип, который выступает как шаблон, от которого объект наследует методы и свойства. Объект-прототип так же может иметь свой прототип и наследовать его свойства и методы и так далее. Это часто называется <span>цепочкой прототипов</span> и объясняет почему одним объектам доступны свойства и методы которые определены в других объектах.</p> <p>Точнее, свойства и методы определяются в свойстве prototype функции-конструктора объектов, а не в самих объектах.</p> <p>В JavaScript создаётся связь между экземпляром объекта и его прототипом (свойство __proto__, которое является производным от свойства prototype конструктора), а свойства и методы обнаруживаются при переходе по цепочке прототипов.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>В JavaScript вызвать функцию можно несколькими способами, в зависимости от типа функции и контекста её вызова.</p> <p><span>1. Обычный вызов (Прямой вызов, Function Declaration)</span></p> <p>Это самый распространённый способ вызова функции.</p> <code>function sayHello() {<br>   console.log('Привет!');<br> }<br> <br> sayHello(); // Выведет: Привет!</code> <p>Если функция принимает аргументы:</p> <code>function greet(name) {<br>   console.log('Привет, ' + name + '!');<br> }<br> <br> greet('Алиса'); // Выведет: Привет, Алиса!</code> <p><span>2. Вызов через переменную (Function Expression)</span></p> <p>Когда функция присвоена переменной, её вызывают через имя переменной.</p> <code>const greet = function(name) {<br>   console.log('Привет, ' + name + '!');<br> };<br> <br>greet('Боб'); // Привет, Боб!</code> <p><span>3. Вызов стрелочной функции</span></p> <p>Аналогично функциональному выражению:</p> <code>const sum = (a, b) => a + b;<br><br>console.log(sum(2, 3)); // 5</code> <p><span>4. Вызов метода объекта (this)</span></p> <p>Функция может быть методом объекта:</p> <code>const user = {<br>   name: 'Оля',<br>   sayHi() {<br>     console.log('Привет, ' + this.name + '!');<br>   }};<br> user.sayHi(); // Привет, Оля!</code> <p><span>5. Вызов в качестве конструтора</span></p> <p>Когда функция вызывается через new, создаётся новый объект, который становится this внутри функции. Если в функции нет return, по умолчанию она возвращает этот новый объект.</p> <code>function User(name, age) {<br>  this.name = name;<br>  this.age = age;<br>  this.sayHello = function() {<br>    console.log(`Привет, меня зовут ${this.name} и мне ${this.age} лет.`);<br>};}<br// Вызов через new создаёт новый объект <br> const user1 = new User('Алиса', 25); <br> user1.sayHello(); // Привет, меня зовут Алиса и мне 25 лет </code> <p><span>6. Вызов через call(), apply(), bind()</span></p> <p>Эти методы позволяют изменить this внутри функции.</p> <code>function greet() {<br>   console.log('Привет, ' + this.name);<br> }<br> const user = { name: 'Иван' };<br> greet.call(user);  // Привет, Иван<br> greet.apply(user); // Привет, Иван<br> const boundGreet = greet.bind(user);<br> boundGreet(); // Привет, Иван</code> <p><span>7. Вызов немедленно вызываемой функции (IIFE)</span></p> <p>Функция вызывается сразу после определения:</p> <code>(function() {<br>  console.log('Я вызвалась сразу!');<br>})();// Я вызвалась сразу!</code> <p><span>8. Вызов функции через setTimeout и setInterval</span></p> <code>setTimeout(() => console.log('Прошла 1 секунда!'), 1000);<br><br>setInterval(() => console.log('Проходит каждые 2 секунды!'), 2000);</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Метод <span>Function.prototype.apply()</span> используется для вызова функции с явным указанием this и передачей аргументов в виде массива или массивоподобного объекта. Синтаксис: <span>func.apply(thisArg, [argsArray])</span>, где <span>thisArg</span> — значение, которое будет использоваться в качестве this при вызове функции, <span>argsArray</span> — массив или массивоподобный объект, содержащий аргументы для вызова функции.</p> <code>const details = {<br>     message: 'Hello World!'<br> }<br> function getMessage() {<br>     return this.message<br> }<br> getMessage.apply(details) // Hello World!</code> <p>Этот метод похож на Function.prototype.call. Единственное отличие состоит в том, что в apply аргументы передаются в виде массива.</p> <code>const person = {<br>     name: 'Marko Polo'<br> }<br>function greeting(greetingMessage) {<br>     return `${greetingMessage} ${this.name}`<br> }<br> greeting.apply(person, ['Hello']) // Hello Marko Polo</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Метод <span>Function.prototype.call()</span> используется для вызова функции с явным указанием this и передачей аргументов в виде массива или массивоподобного объекта. Синтаксис: <span>func.apply(thisArg, [argsArray])</span>, где <span>thisArg</span> — значение, которое будет использоваться в качестве this при вызове функции, <span>argsArray</span> — массив или массивоподобный объект, содержащий аргументы для вызова функции.</p> <code>const details = {<br>     message: 'Hello World!'<br> }<br> function getMessage() {<br>     return this.message<br> }<br> getMessage.apply(details) // Hello World!</code> <p>Этот метод похож на Function.prototype.apply. Единственное отличие состоит в том, что в call аргументы передаются через запятую.</p> <code>const person = {<br>     name: 'Marko Polo'<br> }<br>function greeting(greetingMessage) {<br>     return `${greetingMessage} ${this.name}`<br> }<br> greeting.apply(person, 'Hello') // Hello Marko Polo</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p><span>Методы call, apply и bind</span> являются частью языка JavaScript и используются для управления контекстом выполнения функций. Вот их основные различия:</p> <p><span>call:</span> Этот метод вызывает функцию с указанным контекстом и аргументами, переданными в виде отдельных аргументов. Синтаксис метода call выглядит следующим образом: function.call(context, arg1, arg2, ...). При использовании call аргументы передаются в виде списка, разделенного запятыми.</p> <p><span>apply:</span> Этот метод вызывает функцию с указанным контекстом и аргументами, переданными в виде массива. Синтаксис метода apply выглядит следующим образом: function.apply(context, [arg1, arg2, ...]). При использовании apply аргументы передаются в виде массива.</p> <p><span>bind:</span> Этот метод создает новую функцию, привязанную к указанному контексту. Он не вызывает функцию немедленно, а возвращает новую функцию, которую можно вызвать позже. Синтаксис метода bind выглядит следующим образом: function.bind(context). При использовании bind контекст функции фиксируется, и при вызове новой функции этот контекст будет сохраняться.</p> <code>const obj = {<br>   name: 'John',<br>   greet: function(message) {<br>     console.log(`${message}, ${this.name}!`);<br>   }<br> };<br> const otherObj = {<br>   name: 'Jane'<br> };<br> <br>// Использование метода call<br> obj.greet.call(otherObj, 'Hello'); // Вывод: Hello, Jane!<br> <br>// Использование метода apply<br> obj.greet.apply(otherObj, ['Hi']); // Вывод: Hi, Jane!<br> <br>// Использование метода bind<br> const boundGreet = obj.greet.bind(otherObj);<br> boundGreet('Hey'); // Вывод: Hey, Jane!</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p><span>Метод bind</span> создает новую функцию, привязанную к указанному контексту. Он не вызывает функцию немедленно, а возвращает новую функцию, которую можно вызвать позже. Синтаксис метода bind выглядит следующим образом: <span>function.bind(context)</span>. При использовании <span>bind</span> контекст функции фиксируется, и при вызове новой функции этот контекст будет сохраняться.</p> <code>const obj = {<br>   name: 'John',<br>   greet: function(message) {<br>     console.log(`${message}, ${this.name}!`);<br>   }<br> };<br> const otherObj = {<br>   name: 'Jane'<br> };<br> <br>// Использование метода call<br> obj.greet.call(otherObj, 'Hello'); // Вывод: Hello, Jane!<br> <br> const boundGreet = obj.greet.bind(otherObj);<br> boundGreet('Hey'); // Вывод: Hey, Jane!</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      ...
      linkName: "",
      answer1:
        "<p>Изменить контекст функции можно с помощью методов <span>bind()</span>, <span>call()</span> и <span>apply()</span>.</p> <p>Пример:</p> <code>function greet() {<br>     console.log('Привет, ' + this.name);<br> }<br> <br>const user = { name: 'Иван' };</code> <code>function introduce(age, city) {<br>     console.log(`Меня зовут ${this.name}, мне ${age} лет, я из ${city}`);<br> }<br> <br>const person = { name: 'Анна' };</code> <p>Метод <span>bind()</span> возвращает новую функцию с привязанным контекстом:</p> <code>const boundGreet = greet.bind(user); <br> boundGreet(); // 'Привет, Иван'</code> <code>const boundIntroduce = introduce.bind(person, 25);<br> boundIntroduce('Москва');<br> // 'Меня зовут Анна, мне 25 лет, я из Москва'</code> <p>bind(person, 25) фиксирует this и первый аргумент (age = 25).</p> <p>Метод <span>call() </span>принимает последовательность аргументов в качестве второго параметра:</p> <code>greet.call(user); // 'Привет, Иван'</code> <p>call(user) заменяет this внутри greet на user.</p> <code>introduce.call(person, 25, 'Москва'); <br>// 'Меня зовут Анна, мне 25 лет, я из Москва'</code> <p><span>apply()</span> принимает массив аргументов в качестве второго параметра:</p> <code>greet.apply(user);</code> <code>introduce.apply(person, [25, 'Москва']);</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      href: "",
      linkName: "",
      answer1: "<p><span>Объектный литерал:</span></p> <code>const o = {<br>     name: 'Mark',<br>     greeting(){<br>         return `Hi, I'm ${this.name}`<br>     }<br> } <br> <br>o.greeting // Hi, I'm Mark</code> <p><span>Функция-конструктор:</span></p> <code>function Person(name){<br>     this.name = name<br> }<br> <br> Person.prototype.greeting = function(){<br>     return `Hi, I'm ${this.name}`<br> }<br> <br> const mark = new Person('Mark')<br> <br> mark.greeting() // Hi, I'm Mark</code> <p><span>Метод Object.create:</span></p> <code>const n = {<br>     greeting(){<br>         return `Hi, I'm ${this.name}`<br>     }<br> }<br> <br> const o = Object.create(n)<br> <br> o.name = 'Mark'<br> <br> console.log(o.greeting) // Hi, I'm Mark</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      href: "",
      linkName: "",
      answer1: "<p>Статья по теме: <a class='question__link' href='https://proglib.io/p/metody-massivov-v-javascript-dlya-novichkov-sovety-rekomendacii-i-primery-2022-04-26' target='_blank' rel='noopener noreferrer'>Методы массивов в JavaScript для новичков: советы, рекомендации и примеры</a></p> <p>Oсновные методы для работы с массивами – <span>forEach</span>, <span>filter</span>, <span>map</span> и <span>reduce</span>.</p> <p>Метод <span>forEach</span> выполняет функцию для каждого элемента в массиве. Он не возвращает ничего, но позволяет выполнять действия с каждым элементом массива.  Применяется, когда нужно выполнить некоторые операции над каждым элементом, но не нужно создавать новый массив:</p> <p><img class='question__img' src='img/11.1proglibPart1.png' alt=''></p> <p>Метод <span>filter</span> создает новый массив, включающий только те элементы исходного массива, для которых функция обратного вызова возвращает <span>true</span>. Используется, когда нужно отфильтровать массив, чтобы включить только определенные элементы:</p> <p><img class='question__img' src='img/11.2proglibPart1.png' alt=''></p> <p>Метод <span>map</span> создает новый массив, который состоит из результатов применения функции к каждому элементу исходного массива. Применяется, когда нужно преобразовать каждый элемент массива:</p> <p><img class='question__img' src='img/11.3proglibPart1.png' alt=''></p> <p>Метод <span>reduce</span> выполняет функцию для каждого элемента массива, накапливая результат в одном значении. Используется, когда нужно объединить все элементы массива в одно значение, например, вычислить сумму всех чисел в массиве</p> <p><img class='question__img' src='img/11.4proglibPart1.png' alt=''></p> ",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      href: "",
      linkName: "",
      answer1: "<p>В JavaScript есть несколько способов определить наличие свойства в объекте.</p> <p><span>Оператор in:</span> Можно использовать оператор in, чтобы проверить наличие свойства в объекте или его прототипе.</p> <p><code>const obj = { name: 'John', age: 30 };<br> <br>  console.log('name' in obj); // true<br> console.log('city' in obj); // false</code></p> <p><span>Метод hasOwnProperty():</span> Метод hasOwnProperty() проверяет, содержит ли объект указанное свойство и не учитывает свойства в прототипе объекта.</p> <p><code>const obj = { name: 'John', age: 30 };<br> <br> console.log(obj.hasOwnProperty('name')); // true <br> console.log(obj.hasOwnProperty('city')); // false</code></p> <p><span>Сравнение со значением undefined:</span> Можно сравнить значение свойства с undefined, чтобы определить его наличие.</p> <p><code>const obj = { name: 'John', age: 30 }; <br> <br> console.log(obj.name !== undefined); // true  <br> console.log(obj.city !== undefined); // false</code></p> <p><span>Использование Object.keys():</span> Можно использовать метод Object.keys() для получения массива всех свойств объекта и затем проверить наличие свойства в этом массиве.</p> <p><code>const obj = { name: 'John', age: 30 }; <br> <br> console.log(Object.keys(obj).includes('name')); // true <br> console.log(Object.keys(obj).includes('city')); // false</code></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      href: "",
      linkName: "",
      answer1: "<p>Отличие состоит в том, что <span>оператор «in»</span> проверяет наличие свойства не только в самом объекте, но и в его прототипах, возвращает true, если свойство где-то есть (даже в прототипе). А <span>метод hasOwnProperty</span> — только в объекте. Возвращает true, только если свойство есть непосредственно в объекте.</p> <p><img class='question__img' src='img/habr70/63.3habr70.png' alt=''></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      href: "",
      linkName: "",
      answer1: "<p>В JavaScript есть два основных способа доступа к свойствам объекта: <span>статический</span> (с использованием точечной нотации) и <span>динамический</span> (с использованием квадратных скобок).</p> <p>Точечная нотация позволяет напрямую получить доступ к свойству объекта, используя <span>имя</span> свойства. Скобочная нотация позволяет динамически получить доступ к свойству объекта с использованием квадратных скобок:</p> <p><img class='question__img' src='img/10.1proglibPart1.png' alt=''></p> <p>Скобочная нотация использует интерполяцию и особенно полезна, если имя свойства неизвестно заранее или когда оно хранится в переменной:</p> <p><img class='question__img' src='img/10.2proglibPart1.png' alt=''></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      href: "",
      linkName: "",
      answer1: "<p>В JavaScript для получения списка ключей и значений объекта используются методы<span>Object.keys() и Object.values()</span>.</p>  <p><span>Object.keys()</span> возвращает массив со всеми ключами объекта:</p> <code>const person = {<br>   name: 'Егор',<br>   age: 30,<br>   city: 'Ростов'<br> };<br> <br> const keys = Object.keys(person);<br> console.log(keys); // ['name', 'age', 'city']</code> <br> <p><span>Object.values()</span> возвращает массив со всеми значениями свойств объекта:</p> <code>const person = {<br>   name: 'Егор',<br>   age: 30,<br>   city: 'Ростов'<br> };<br> <br> const values = Object.values(person);<br> console.log(values); // ['Егор', 30, 'Ростов']</code> <p>Используя Object.keys() и Object.values(), можно сделать перебор:</p> <code>const person = {<br>   name: 'Егор',<br>   age: 30,<br>   city: 'Ростов'<br> };<br> <br> Object.keys(person).forEach(key => {<br>   console.log(key, person[key]); <br> });</code> <p>Или вывести ключи и значения в виде объекта:</p> <code>const person = {<br>   name: 'Егор',<br>   age: 30,<br>   city: 'Ростов'<br> };<br> <br> Object.entries(person).forEach(([key, value]) => {<br>   console.log(`${key}: ${value}`); <br> });</code>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      href: "",
      linkName: "",
      answer1: "<p>Для такой проверки можно использовать встроенный метод <span>Array.isArray()</span>. Этот метод принимает объект в качестве аргумента и возвращает <span>true</span>, если объект является массивом, и <span>false</span> в противном случае:</p> <p><img class='question__img' src='img/15proglibPart1.png' alt=''></p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      href: "",
      linkName: "",
      answer1: "<p>Чтобы добавить элемент в начало массива, можно использовать Array.prototype.unshift(). Для добавления элемента в конец массива подойдет Array.prototype.push().</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
  ...
  },
  {
    ...
    answer: {
      href: "",
      linkName: "",
      answer1: "<p><span>1. Поверхностное копирование (shallow copy)</span></p> <p>Копируются только <span>верхние уровни</span> объекта. Вложенные объекты всё равно передаются <span>по ссылке</span>.</p> <p><span style='font-style: italic;'>- Через Object.assign():</span></p> <code>const obj = { a: 1, b: { c: 2 } };<br> const clone = Object.assign({}, obj); </code> <p><span style='font-style: italic;'>- Через оператор расширения (spread ...):</span></p> <code>const obj = { a: 1, b: { c: 2 } };<br> const clone = { ...obj };</code> <p>Но если поменять вложенный объект:</p> <code>clone.b.c = 42; <br> console.log(obj.b.c); // 42 (так как b — это ссылка на тот же объект!)</code> <p><span>Поверхностное копирование не защищает от изменений вложенных структур!</span></p> <p><span>2. Глубокое копирование (deep copy)</span></p> <p><span style='font-style: italic;'></span></p> <p><span style='font-style: italic;'>- Через JSON:</span></p> <code>const obj = { a: 1, b: { c: 2 } }; <br> const deepClone = JSON.parse(JSON.stringify(obj));</code> <p>❗ Недостатки: Теряются функции (function), undefined, Symbol, и объекты типа Date, Map, Set превращаются в простые данные или теряются.</p> <p><span>- С помощью библиотек:</span></p> <p>Например, lodash:</p> <code>import cloneDeep from 'lodash/cloneDeep';<br> <br> const obj = { a: 1, b: { c: 2 } };<br> const deepClone = cloneDeep(obj);</code> <p>Это более надёжный способ для сложных объектов.</p>",
      image1: "",
      href2: "",
      linkName2: "",
      answer2: "",
...
const res1 = new Boolean(""); //[Boolean: false]
const res2 = new Boolean(0); //[Boolean: false]
const res3 = new Boolean(null); //[Boolean: false]
const res4 = new Boolean(undefined); //[Boolean: false]
const res5 = new Boolean(NaN); //[Boolean: false]
const res6 = new Boolean(false); //[Boolean: false]
